...
this.platform.log(`currentIntensity  '${currentIntensity}'`);
...
*/
addAccessory = (name) => {
  const {UUIDGen, Accessory } = this.platform;
      // Gets the main light bulb accessory
      let accessory = this.unusedDeviceAccessories.find(function(a) { return a.context.kind === name; });
      if (accessory) {
...
setUpModeSwitch = (accessory, name, mode) => {
  ...
  service.getCharacteristic(Characteristic.On).onGet(() => this.isOn(mode));
  service.getCharacteristic(Characteristic.On).on('set', (value, callback) => {
      // Saves the changes
      if (value) {
          this.platform.log.debug('Switch state to video');
          this.platform.limiter.schedule(() => { return this.platform.client.updateExecution({ 'mode': mode || this.getLastSyncMode() }); }).then(() =>{}, () => {
  ...
  this.onUpdate(() => {
    this.platform.log(mode, this.isOn(mode))
    service.getCharacteristic(Characteristic.On).updateValue(this.isOn(mode));
  })
}
...
setUpIntensitySwitch = (accessory, name, intensity) => {
  ...
  service.getCharacteristic(Characteristic.On).onGet(() => this.isIntensity(intensity));
  service.getCharacteristic(Characteristic.On).on('set', (value, callback) => {
      // Saves the changes
      if (value) {
          this.platform.log.debug(`Switch intensity to ${intensity}`);
          this.platform.limiter.schedule(() => { return this.platform.client.updateExecution({ 'intensity': intensity }); }).then(() =>{
  ...
  this.onUpdate(() => {
    service.getCharacteristic(Characteristic.On).updateValue(this.isIntensity(intensity));
  })
}
...
setUpInputSwitch = (accessory, name, inputName) => {
  ...
  service.getCharacteristic(Characteristic.On).onGet(() => this.isInput(inputName));
  service.getCharacteristic(Characteristic.On).on('set', (value, callback) => {
      // Saves the changes
      if (value) {
          this.platform.log.debug(`Switch input to ${inputName}`);
          this.platform.limiter.schedule(() => { return this.platform.client.updateExecution({'hdmiSource': inputName}); }).then(() =>{
  ...
  this.onUpdate(() => {
    service.getCharacteristic(Characteristic.On).updateValue(this.isInput(inputName));
  })
}
